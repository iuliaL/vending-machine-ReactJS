...
export default class Machine extends React.Component{
	constructor (props) {
		...
		this.reset = debounce(this.reset,2500).bind(this);
	}
	getAll(){
    ...
    pay(){
    	service.updateStock(this.productType)
		    .then((res) => {
    ...
    deliver(){
	    ...
	    console.log('deliver paid', this.state.paid);
	    this.productType = null; // reset machine;
	    this.reset();
    }
    reset(){
	    console.log('reset paid', this.state.paid);
		...
		return (
			<div className="panel panel-danger text-center">
				...
				<div className="panel-footer">
					{!this.productType && !this.state.busy &&<p className="status">Select Product</p>}
					{this.productType && !this.state.paid && <button className="btn btn-default" onClick={this.pay}>Pay amount</button>}
